...
public class BytesUtil {
     ...
     * @param bigMode 是否是大端模式，大端模式，高字节在前面
     * @return byte数组
     */
    public static byte[] intToBytes(int value, boolean bigMode) {
     ...
     * @param length  一共需转换几个字节
     * @param bigMode 是否是大端模式，大端模式，高字节在前面
     * @return int数值
     */
    public static int bytesToInt(byte[] src, int offset, int length, boolean bigMode) {
        int value;
        if (bigMode) {
            int start = offset + length - 1;
            value = src[start] & 0xFF;
            for (int i = 1; i < length; i++) {
                value |= ((src[start - i] & 0xFF) << (8 * i));
            }
        } else {
    ...
    public static int bytesToInt(byte[] src, boolean bigMode) {
        return bytesToInt(src, 0, 4, bigMode);
    }
